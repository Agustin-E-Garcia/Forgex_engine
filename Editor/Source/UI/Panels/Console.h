    ...
    protected:
        void OnDraw() override
        {
            ...
            if (ImGui::InputText("Input", m_CommandBuffer, IM_COUNTOF(m_CommandBuffer), input_text_flags))
            {
                ...
                {
                    GET_SERVICE(Debug::DebugManager)->GetCommandHandler()->Execute(std::string(m_CommandBuffer));
                    m_CommandBuffer[0] = '\0';
                }
        ...
        void OnUpdate() override
        {
            ...
            while (GET_SERVICE(Debug::DebugManager)->GetLogManager()->GetNextConsoleMessage(message, level))
            {
